...
from losses_evid import edl_digamma_loss, softplus_evidence
...
def produce_evaluation_file_multi_exit(
    ...
    fname_list = []
    score_list = []
    gat_S_exit_score_list = []
    gat_T_exit_score_list = []
    ST_exit_score_list = []
    u_list = []
    u_S_exit_list = []
    u_T_exit_list = []
    ...
    for batch_x, utt_id in data_loader:
        ...
        with torch.no_grad():
            # Multi exit
            _, batch_out, gat_S_exit, gat_T_exit, ST_exit = model(batch_x)

            # Evid
            evidence = softplus_evidence(batch_out)
            alpha = evidence + 1
            u_final = 2 / torch.sum(alpha, dim=1, keepdim=True).squeeze(-1)
            prob = alpha / torch.sum(alpha, dim=1, keepdim=True)
            batch_score = (prob[:, 1]).data.cpu().numpy().ravel()

            evidence = softplus_evidence(gat_S_exit)
            alpha = evidence + 1
            u_S_exit = 2 / torch.sum(alpha, dim=1, keepdim=True).squeeze(-1)
            prob = alpha / torch.sum(alpha, dim=1, keepdim=True)
            gat_S_exit_score = (prob[:, 1]).data.cpu().numpy().ravel()

            evidence = softplus_evidence(gat_T_exit)
            alpha = evidence + 1
            u_T_exit = 2 / torch.sum(alpha, dim=1, keepdim=True).squeeze(-1)
            prob = alpha / torch.sum(alpha, dim=1, keepdim=True)
            gat_T_exit_score = (prob[:, 1]).data.cpu().numpy().ravel()

            evidence = softplus_evidence(ST_exit)
            alpha = evidence + 1
            u_ST_exit = 2 / torch.sum(alpha, dim=1, keepdim=True).squeeze(-1)
            prob = alpha / torch.sum(alpha, dim=1, keepdim=True)
        ...
        gat_S_exit_score_list.extend(gat_S_exit_score.tolist())
        u_S_exit_list.extend(u_S_exit.tolist())

        gat_T_exit_score_list.extend(gat_T_exit_score.tolist())
        u_T_exit_list.extend(u_T_exit.tolist())

        ST_exit_score_list.extend(ST_exit_score.tolist())
        u_ST_exit_list.extend(u_ST_exit.tolist())
    ...
    with open(T_exit_save_path, "w") as fh:
        for fn, sco, trl, u in zip(fname_list, gat_T_exit_score_list, trial_lines, u_T_exit_list):
            _, utt_id, _, src, key = trl.strip().split(' ')
            assert fn == utt_id
            fh.write("{} {} {} {} {}\n".format(utt_id, src, key, sco, u))

    with open(ST_exit_save_path, "w") as fh:
        for fn, sco, trl, u in zip(fname_list, ST_exit_score_list, trial_lines, u_ST_exit_list):
            _, utt_id, _, src, key = trl.strip().split(' ')
            assert fn == utt_id
            fh.write("{} {} {} {} {}\n".format(utt_id, src, key, sco, u))
...
def produce_evaluation_file(
    ...
    for batch_x, utt_id in data_loader:
        batch_x = batch_x.to(device)
        with torch.no_grad():
            _, batch_out, _, _, _ = model(batch_x)
